...
class TransformerNetwork(nn.Module):
    ...
    def forward_actor(self, features: th.Tensor) -> th.Tensor:
        transformer_out=self.__get_transformer_latest_out(features)
    ...
    def __get_transformer_latest_out(self, features: th.Tensor) -> th.Tensor:
        """
        Transformerに通して最新の隠れ層の出力を取得する関数
        """
        ...
        transformer_in=transformer_in+self.positional_encoding.to(features.device) # 位置エンコーディングを加算
        ...
        return transformer_out[:,-1,:] # 最終stepだけ取り出す


class TransformerNetworkPolicy(ActorCriticPolicy):
    def __init__(
        self,
    ...
    def _build_mlp_extractor(self) -> None:
        ...
        self.mlp_extractor = TransformerNetwork(
            self.features_dim, 
