...
def test_env(env:CryptoEnv, policy:DummyPolicy, n_test:int=1, save_path:Path=None):
    results=[]

    print(f"\033[32m--- START test_env "+"-"*100+"\033[0m")
    for i_test in tqdm(range(n_test), desc="test_env"):
        ...
        while not done:
            ...
            realized_pnl_rate_history.append(env.agent.cumlative_realized_pnl_rate)
    ...
    print(f"\033[32m--- END test_env --"+"-"*100+"\033[0m")
